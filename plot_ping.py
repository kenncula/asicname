...
def make_figure(data, probes, latency_type):
    ...
    for key, value in sorted_data.items():
        ...
        probe = probes[key]
        urls = [
            'https://atlas.ripe.net/probes/' + str(key) + '/' for i in range(l)
        ]
        ...
        if probe['asn_v4'] == 14593 or probe['asn_v6'] == 14593:
            fig.add_trace(
                go.Scatter(
                    ...
                    name='Probe #' + str(key),
                    mode='lines+markers',
                    hovertemplate='<b>' + latency_type.capitalize() +
                    ' Latency</b>: %{y:.2f} ms' +
                    '<br><b>Time</b>: %{x} UTC<br>' + '%{text}<extra></extra>',
                    text=[
                        '<b>Probe ID</b>: ' + str(probe['id']) +
                        '<br><b>Description</b>: ' +
                        str(probe['description']) + '<br><b>Country</b>: ' +
                        pycountry.countries.get(
                            alpha_2=probe['country_code']).name + ' ' +
                        str(flag.flag(probe['country_code'])) +
                        "<br> <b>CLICK POINT FOR MORE INFO</b>"
                        for i in range(l)
                    ],
                    customdata=urls))
        else:
            print("SKIPPING Probe ID " + str(probe['id']) +
                  " with ASN ID of " + str(probe['asn_v4']) + '/' +
                  str(probe['asn_v6']))

    fig.update_layout(title=latency_type.capitalize() +
                      " Latency Over Time of RIPE Atlas Starlink Probes",
...
@app.callback(Output('avg-graph', 'figure'), [Input('avg-graph', 'clickData')])
...
@app.callback(Output('min-graph', 'figure'), [Input('min-graph', 'clickData')])
...
@app.callback(Output('max-graph', 'figure'), [Input('max-graph', 'clickData')])
